...
export class MysqlUserRepository implements userRepository {
    ...
    async getById(uuid: string): Promise<User | null> {
        try {
            ...
            if (rows.length > 0) {
                const user = rows[0];
                return new User(user.uuid, user.name, user.lastName, user.email, user.tel, user.status, user.password);
            } else {
                return null;
            }
        } catch (error) {
    ...
    async allUsersDisabled(): Promise<User[] | null> {
        try {
            ...
            return rows.map((row: any) => new User(row.uuid, row.name, row.lastName, row.email, row.tel, row.status, row.password));
        } catch (error) {
